...
https://developer.enonic.com/docs/platform/xp7/apps[Enonic apps^] are used to ship everything from content models to code and platform extensions, which you will soon discover.
...
*Open a new terminal window* and create your first Enonic app by running this command:
  ...
  enonic project create com.example.myapp -r tutorial-intro -b xp7 -s mysandbox

This command will:

* use the GitHub repo `github.com/enonic/tutorial-intro` as a starter based on the `xp7` branch
...
<4> https://developer.enonic.com/docs/platform/xp7/framework/controllers[Server-side controllers and templates^]
<5> https://developer.enonic.com/docs/platform/xp7/framework/i18n[Localisation bundles^]
<6> Sample content
<7> https://developer.enonic.com/docs/platform/xp7/cms/content-types[Content Type schemas^]
<8> https://developer.enonic.com/docs/platform/xp7/cms/x-data[X-data schemas^]
